...
if ($_GET['mapsId'] != '') {
   $showMaps = mysqli_fetch_array(mysqli_query($conn, 'SELECT * FROM mapsgoo WHERE mps_id=' . $_GET['mapsId']));
   $name = $showMaps['mps_name'];
   $lat = $showMaps['mps_lat'];
   $lng = $showMaps['mps_lng'];
   $zoom = $showMaps['mps_zoom'];
} else {
   header('Location:controller/google-map-api.php');
   exit();
}
?>  
